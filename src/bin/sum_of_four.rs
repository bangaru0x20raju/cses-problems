...
fn main() {
    ...
    for i in 0..n_and_k[0] {
        for j in i+1..n_and_k[0] {
            let mut left = (j as usize) + 1;
            let mut right: usize = (n_and_k[0] as usize) - 1;
            while left < right {
                let temp_sum: u64 = numbers_with_index[i as usize].0
                    + numbers_with_index[left].0
                    + numbers_with_index[right].0
                    + numbers_with_index[j as usize].0;
                if temp_sum > n_and_k[1] {
                    ...
                    println!(
                        "{} {} {} {}",
                        numbers_with_index[i as usize].1,
                        numbers_with_index[left].1,
                        numbers_with_index[right].1,
                        numbers_with_index[j as usize].1
                    );
                    return;
                }
            }
